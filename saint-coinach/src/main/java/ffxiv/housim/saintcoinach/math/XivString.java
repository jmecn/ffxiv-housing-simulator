...
public final class XivString {
	...
	public static String parseFFXIVString(byte[] stringBytes) {
		...
		try{
			...
			return new String(newStringBytes, 0, buffOut.position(), UTF_8);
		} catch (Exception e) {
			return "<ERROR Parsing: "+ new String(stringBytes, UTF_8) + ">";
		}
	}
	...
	private static void processPacket(ByteBuffer buffIn, ByteBuffer buffOut) {
		...
		case TYPE_INFO:
			if ((((int)payload[0])&0xFF) == 0xEB && (((int)payload[1])&0xFF) == 0x02)
				buffOut.put("<forename surname>".getBytes(UTF_8));
			else
				buffOut.put("<value>".getBytes(UTF_8));
			break;
		case TYPE_SPLIT:
			
			if (Arrays.equals(payload, forenamePayload))
			{
				buffOut.put("<forename>".getBytes(UTF_8));
				break;
			}
			else if (Arrays.equals(payload, surnamePayload))
			{
				buffOut.put("<surname>".getBytes(UTF_8));
				break;
			}
			
			buffOut.put("<split:".getBytes(UTF_8));

			int contentsSize = payload[1];

			buffOut.put(String.format("[%d]", payload[1 + contentsSize + 1]).getBytes(UTF_8));
			...
			buffOut.put((new String(outSplitProcessBuffer, 0, outSplitProcessBuffer.length, UTF_8).getBytes(UTF_8)));

			buffOut.put(">".getBytes(UTF_8));
			break;
		case 222:
		...
		case TYPE_COLOR_CHANGE:
			if (payload[0] == -20)
				buffOut.put("</color>".getBytes(UTF_8));
			else if (payload[0] == -2)
				buffOut.put(String.format("<color #%02X%02X%02X>", payload[2], payload[3], payload[4]).getBytes(UTF_8));
			else buffOut.put("<color?>".getBytes(UTF_8));
			break;
		case TYPE_LINK:
			...
			buffOut.put(String.format("<2b?:0x%x, 0x%x, 0x%x>", payload[0], payload[1], payload[2]).getBytes(UTF_8));

			break;
		case TYPE_REFERENCE:
			byte[] exdName = new byte[payload[1]-1];
			System.arraycopy(payload, 2, exdName, 0, exdName.length);
			buffOut.put(String.format("<ref:%s>", new String(exdName)).getBytes(UTF_8));
			break;
		...
		case TYPE_IF:
			...
			buffOut.put(builder.toString().getBytes(UTF_8));
		...
		case TYPE_SWITCH:
				...
				while (true)
				{
					...
					if (stringSize-1 != 0){
						...
						if (switchBuffer[0] == 0x02)
						{
							...
							switchString2.append(new String(outProcessBuffer, 0, outProcessBB.position(), UTF_8));
						}
						else
							switchString2.append(new String(switchBuffer, UTF_8));
					}
					pos2+=stringSize-1;
			...
			switchString2.append(">");
			buffOut.put(switchString2.toString().getBytes(UTF_8));
			break;
		case TYPE_NEWLINE:
			buffOut.put("\\n".getBytes(UTF_8));
			break;
		case TYPE_ITEM_LOOKUP:
			buffOut.put("<item>".getBytes(UTF_8));
			break;
		case TYPE_ICON1:
		case TYPE_ICON2:
			buffOut.put(String.format("<icon:%d>", payload[0]).getBytes(UTF_8));
			break;
		case TYPE_DASH:
			buffOut.put("-".getBytes(UTF_8));
			break;
		...
		default:
			...
			buffOut.put(unknownMsg.getBytes(UTF_8));
			break;
		}
